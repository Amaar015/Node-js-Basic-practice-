...
//  Insert();
     ...
     const find=async()=>{
                 const results5 =await School_employee.find({status:"Active",age:{$gt:30}}).select({id:0});
                   const results4 =await School_employee.find({status:"Active",age:{$lt:30}}).select({id:0});
                const results3 =await School_employee.find({status:"Active",age:{$gte:30}}).select({id:0});
                const results2 =await School_employee.find({status:"Active",age:{$lte:30}}).select({id:0});
                const results1 =await School_employee.find({status:"Active",age:{$in:30}}).select({id:0});
